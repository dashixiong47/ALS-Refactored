#include "Nodes/AlsRigUnit_DistributeSpineAdvanced2.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AlsRigUnit_DistributeSpineAdvanced2)

FAlsRigUnit_DistributeSpineAdvanced2_Execute()
{
	...
	// --- Pitch 处理 ---
	if (bEnablePitchRotation && !FMath::IsNearlyZero(PitchAngle, KINDA_SMALL_NUMBER))
	{
	...
	// --- Yaw 处理 ---
	if (SpineYawBone.IsValid() && !FMath::IsNearlyZero(YawAngle, KINDA_SMALL_NUMBER))
	{
